...
class paso_actualizar_destino extends paso
{
	protected $path;
	...
	function verificar_carpeta_writable($path, $extra=true)
	{
		...
		if ($ok && ! file_exists($path)) {
			$this->set_error('no_existe', "Imposible acceder al directorio '{$path}'. Verifique que el mismo exista y que el usuario $usuario posea permisos sobre el mismo.");
			$ok = false;
		}
		if ($ok && ! is_dir($path)) {
			$this->set_error('no_es_directorio', "'{$path}' es un archivo regular y no un directorio.");
			$ok = false;
		}
