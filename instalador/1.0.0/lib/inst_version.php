...
class inst_version
{
	...
	function __construct($numero)
	{
		...
		if (count($this->partes) < 3) {
			throw new inst_error("El número de versión $numero es incorrecto. Se requiere al menos 3 partes. ".$formato);
		}
		if (!is_numeric($this->partes[2])) {
			$digito = intval($this->partes[2]);
			if (! is_numeric($digito)) {
				throw new inst_error("El número de versión $numero es incorrecto. Las partes deben ser numéricas. ".$formato);
			}
			...
			if (isset($inestable)) {
				if (! in_array($inestable, $this->inestables)) {
					throw new inst_error("El número de versión $numero es incorrecto. El codigo de inestable '$inestable' no es válido. ".$formato);
				}
			...
			if ($build != '') {
				$this->build = $build;				
				if (! is_numeric($build)) {
					throw new inst_error("El número de versión $numero es incorrecto. El codigo de build '$build' no es válido. ".$formato);					
				}
			}
		}
