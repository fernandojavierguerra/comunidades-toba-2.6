 ...
 */
class auditoria_tablas_postgres
{
	...
	function __construct($conexion, $schema_origen='public', $schema_logs='public_auditoria', $schema_toba=null) 
	{
	...
	function generar($tablas, $prefijo_tablas)
	{
		...
		if (file_exists($archivo_datos)) {
			inst::db_manager()->ejecutar_archivo($this->conexion, $archivo_datos);
		}
	...
	function agregar_tablas($prefijo=null)
	{
		$where = '';
		if (isset($prefijo)) {
			$prefijo_sano = inst::db_manager()->quote($this->conexion, "$prefijo%");
			$where .= " AND (tablename LIKE $prefijo_sano)";
		}
		$schema =  inst::db_manager()->quote($this->conexion, $this->schema_origen);
		$sql = "
		...
		";
		 $aux = inst::db_manager()->consultar($this->conexion, $sql);
		 foreach ($aux as $t) {
	...
	function crear()
	{
		...
		//-- Schema de toba
		if (isset($this->schema_toba) && inst::db_manager()->existe_schema($this->conexion, $this->schema_toba)) {
	...
	function purgar($lapso_tiempo = 0)
	{
		foreach($this->tablas as $t) {
			...
			if (inst::db_manager()->existe_tabla($this->conexion, $this->schema_logs, $nombre)) {
	...
	function get_triggers_activos()
	{
		...
		$triguers = inst::db_manager()->get_triggers_schema($this->conexion, $this->schema_origen, 'tauditoria_', 'O');
		return $triguers;
	}
	...
	function existe()
	{
		$schema =  inst::db_manager()->quote($this->conexion, $this->schema_logs);
		$sql = "
			SELECT nspname
			FROM pg_catalog.pg_namespace
			WHERE nspname = $schema;
		";
		$r = inst::db_manager()->consultar($this->conexion, $sql);
		return (! empty($r));
	}
	...
	{
		inst::db_manager()->crear_schema($this->conexion, $this->schema_logs);
	}
	
	protected function crear_lenguaje($lenguaje='plpgsql')
	{
		inst::db_manager()->crear_lenguaje($this->conexion, $lenguaje);
	}
	...
	{
		$db_mgr = inst::db_manager();
		$campos = $db_mgr->get_definicion_columnas($this->conexion, $t, $schema);
	...
	protected function actualizar_tabla($origen, $schema)
	{
		...
		$db_mgr = inst::db_manager();		
		$negocio = $db_mgr->get_definicion_columnas($this->conexion, $origen, $schema);
		$negocio[] = array(
		...
		$logs = $db_mgr->get_definicion_columnas($this->conexion, $destino, $this->schema_logs);
	...
	function get_datos($tabla, $filtro = array())
	{
		...
		$filtro_valores =  inst::db_manager()->quote($this->conexion, $filtro);
		...
		$sql = "SELECT
					aud.*,
					CASE
						WHEN aud.auditoria_operacion = 'U' THEN 'Modificaci�n'
		...
		";
		$datos = inst::db_manager()->consultar($this->conexion, $sql);
		return $datos;
	}
	...
	function get_campos_claves($tabla)
	{
		...
		if (! inst::db_manager()->existe_tabla($this->conexion, $this->schema_origen, $tabla)
			&& isset($this->schema_toba) && inst::db_manager()->existe_schema($this->conexion, $this->schema_toba)) {
			$schema = $this->schema_toba;
		}
		$db_mgr = inst::db_manager();
		$cols = $db_mgr->get_definicion_columnas($this->conexion, $tabla, $schema);
		$pks = array();
		foreach ($cols as $col) {
			if ($col['pk']) {
				$pks[] = $col['nombre'];
	...
	{
		$tiempo =  inst::db_manager()->quote($this->conexion, $lapso_tiempo . ' months' );
